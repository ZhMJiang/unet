...
def unet(pretrained_weights = None,input_size = (256,256,1),use_mirror_padding = False):
    ...
    if use_mirror_padding :
        downPadding = 'valid'
        # def doPadding(x) :
            # return tf.pad(x,tf.constant([[0,0],[62, 62], [62, 62],[0,0]]),"SYMMETRIC")
        # padLayer = Lambda(doPadding)
        # input_p = padLayer(input_s)
        input_p = ComplexPadding2D(padding=62,mode='SYMMETRIC',debug = True)(input_s)
    else :
        downPadding = 'same'
        input_p = input_s
    ...
    if not use_mirror_padding :
        ...
        merge7 = concatenate([Cropping2D(12)(conv3),up7], axis = 3)
    ...
    if not use_mirror_padding :
        ...
        merge8 = concatenate([Cropping2D(28)(conv2),up8], axis = 3)
    ...
    if not use_mirror_padding :
        ...
        merge9 = concatenate([Cropping2D(60)(conv1),up9], axis = 3)
